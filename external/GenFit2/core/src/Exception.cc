...
std::set<std::string> Exception::emessg_;
bool Exception::emessgflag_ = false;
...
Exception::Exception(std::string excString, int line, std::string  file) :
  ...
  if (emessgflag_) addEmessg(excString);
}
...
const char* Exception::getEmessg(int i) 
{  
  if (!emessgflag_) return ""; 
  if (emessg_.empty() || i >= emessg_.size()) return "";
      std::set<std::string>::const_iterator it = emessg_.begin();
      int n = 0;
      while (i != n++) it++;
      return (*it).c_str();
}

void Exception::addEmessg(std::string& msg)
{
