...
//_____________________________________________________________________________ 
Bool_t ClusterCollection::FindElement(const ClusterElement* el) const 
{
   if (GetId() < 1) return kFALSE;
   if (!el) return kFALSE;
   if (Clusters_.empty()) return kFALSE;
     
   CL_CSET::const_iterator it = Clusters_.begin();
   for ( ;it != Clusters_.end(); ++it) {
        if (it->second->FindElement(el)) return kTRUE;
   }
   return kFALSE;
}

//_____________________________________________________________________________ 
Bool_t ClusterCollection::FindElement(Int_t eid) const 
{
   if (GetId() < 1) return kFALSE;
   if (eid < 1) return kFALSE;
   if (Clusters_.empty()) return kFALSE;
     
   CL_CSET::const_iterator it = Clusters_.begin(); 
   for ( ;it != Clusters_.end(); ++it) {
        if (it->second->FindElement(eid)) return kTRUE;
   }
   return kFALSE;
}
