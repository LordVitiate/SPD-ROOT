...
//  run->AddModule(its);
  ...
  SpdPrimaryGenerator* primGen = new SpdPrimaryGenerator(); 
      ...
      isogen1->Initialize(-11,0.1,2);  // pdg number, momentum (GeV/c), particles per event or LEVEL
      ...
      isogen2->Initialize(11,0.1,2);  // pdg number, momentum (GeV/c), particles per event or LEVEL
  ...
  //primGen->SetBeam(50., 50., 0., 0.); // (X,Y)- position, (X,Y)- (0.5*delta or sigma) [cm]
  //primGen->SmearVertexXY(kTRUE);   
      
  //primGen->SetTarget(50.,0.);  // Z- position, 0.5*delta or sigma [cm]
