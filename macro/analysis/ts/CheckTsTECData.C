...
SpdParSet*          TsTECPars_;
SpdPrimGenParSet*   GenPars_;

SpdTsTECGeoMapper*   TsTECGeoMapper_;
SpdTsTECGeoBuilder*  TsTECGeoBuilder_;

SpdMCEventHeader*   Header_      = 0;
TClonesArray*       MCTracks_    = 0;
TClonesArray*       TsTECPoints_ = 0;
...
void AnalyseEvent()
{
   ...
   //--------------------------- TSTEC POINTS -------------------------------
   Int_t ntstb_points = TsTECPoints_->GetEntriesFast();
        
   cout << "\n<Analyse> tstec-points = " << ntstb_points << endl;
   ...
   for (Int_t i(0); i < ntstb_points; i++) {
        ...
        point = (SpdTsTECPoint*)TsTECPoints_->At(i); 
...
void ScanNodeGeo(Long_t tid)
{
   ...
   TsTECGeoBuilder_->SetNodePath(tid);  // set straw node
   
   TString node_path = TsTECGeoBuilder_->GetNodePath(); // get straw "path"
   ...
   if (layer_copy_number != 0) {
       TsTECGeoBuilder_->SetNodePath(layer_path); // set layer node 
       PrintCurrentNode("layer");
   }
   
   if (module_copy_number != 0) {
       TsTECGeoBuilder_->SetNodePath(module_path); // set module node 
...
void PrintCurrentNode(const Char_t* title)
{
   ...
   const TGeoVolume* volume = TsTECGeoBuilder_->GetNodeVolume(); 
   if (volume) TsTECGeoBuilder_->PrintGeoVolume(volume);
   
   const SpdGeoVolPars* volpars = TsTECGeoMapper_->GetVolPars(volume->GetName());
   if (volpars) volpars->PrintGeoVolPars();
   
   const SpdGeoVVolume* vvolume = TsTECGeoMapper_->GetVVolume(volume->GetName());
   if (vvolume) vvolume->PrintGeoVVolume();
   
   Double_t node_pos[3];  // node position (x,y,z)
    
   TsTECGeoBuilder_->GetDetectorPosition(node_pos);  
   printf("%s position (X,Y,Z):    (%12.6f, %12.6f, %12.6f)  [cm, cm, cm]  \n",
           title,node_pos[0],node_pos[1],node_pos[2]);
   
   TsTECGeoBuilder_->GetDetectorPositionRPZ(node_pos);    
   printf("%s position (R,Phi,Z):  (%12.6f, %12.6f, %12.6f)  [cm, deg, cm] \n",
           title,node_pos[0],node_pos[1],node_pos[2]);
   
   Double_t node_dir[3];  // node orientation (x,y,z-axis)
   
   TsTECGeoBuilder_->GetDetectorOrientation(node_dir,'x');  
   printf("%s X-axis:              (%10.6f, %10.6f, %10.6f) \n",
           title,node_dir[0],node_dir[1],node_dir[2]);
   
   TsTECGeoBuilder_->GetDetectorOrientation(node_dir,'y'); 
   printf("%s Y-axis:              (%10.6f, %10.6f, %10.6f) \n",
           title,node_dir[0],node_dir[1],node_dir[2]);
   
   TsTECGeoBuilder_->GetDetectorOrientation(node_dir,'z'); 
...
Bool_t RegisterRunData(TString inFile)
{
  ...
  branch = t->GetBranch("SpdTsTECPoint");
  if (!branch) return kFALSE;
  t->SetBranchAddress("SpdTsTECPoint",&TsTECPoints_) ;
...
Bool_t LoadParameters(TString inParFile)
{
  ...
  /* INNER TRACKER SYSTEM (TSTEC) PARAMETERS */
  
  TsTECPars_ = (SpdParSet*)fp->Get("TsTECParSet");
  if (!TsTECPars_) {
      cout << "No TSTEC parameters " << endl;
      return kFALSE;
  }
  
  //TsTECPars_->print(1);
 
  /* +++++++++++++++ EXTRACT TSTEC GEOMETRY  +++++++++++++++ */
  ...
  SpdDetector* det = SpdCommonGeoMapper::Instance()->SearchForActive(kSpdTsTEC); 
  if (!det) return kFALSE;
 
  det->LoadParsFrom(TsTECPars_);
  det->ConstructGeometry();
  
  TsTECGeoMapper_  = dynamic_cast<SpdTsTECGeoMapper*>(det->GetMapper());
  TsTECGeoBuilder_ = dynamic_cast<SpdTsTECGeoBuilder*>(det->GetBuilder());
  
  if (!TsTECGeoMapper_ || !TsTECGeoBuilder_) return kFALSE;
  
  GeoLevel_ = TsTECGeoBuilder_->GetActualGeoLevel();
  
  TsTECGeoMapper_->Print(""); 
  //TsTECGeoMapper_->PrintVolPars();
  //TsTECGeoBuilder_->Print(""); 
