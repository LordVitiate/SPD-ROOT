...
void reco(int nEvents = 0)
{
  ...
  double mkm = 1.e-4; // to cm
//  its_hits_producer->SetHitResolution('s', 80*mkm,   30*mkm); // set resolution for dssd; the default is (81, 27)
//  its_hits_producer->SetHitResolution('m', 5*mkm,   5*mkm);   // set resolution for maps; the default is 10
