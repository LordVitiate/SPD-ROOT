...
void MakeIdealTracks() 
{
  ...
  TrackFinder->SetAbsMinTrackerNHits(0);
  ...
  TrackFinder->SetMinVertexNHits(1);  // its hits number cut; default = 1; default absolute min. value = 0;  
  TrackFinder->SetMinTrackerNHits(2); // tstb+tstec hits number cut; default = 2; default absiolute min. value = 2;  
  ...
  TrackFinder->SetHitResolution(kSpdIts,  'm',  50*mkm,  50*mkm); // BARREL: MAPS   (u=x,v=y)
