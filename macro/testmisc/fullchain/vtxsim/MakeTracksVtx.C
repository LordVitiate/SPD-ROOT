...
void MakeTracksVtx() 
{
  ...
  TString outFile = "TracksVtx.root";   // Output file
  ...
  // absolute min. cut value 
  TrackFinder->SetAbsMinTrackerNHits(0);  // !!!ATTENTION!!!ATTENTION!!!ATTENTION!!!
  
  // set cuts
  TrackFinder->SetMinMomentum(1e-2);  // momentum cut;  default = 1e-2 (10 MeV); default absiolute min. value = 4e-3 (4 MeV)
  TrackFinder->SetMinGammaBeta(0.5);  // p/m cut; default = 0.5; default absiolute min. value = 0.05
  TrackFinder->SetMinVertexNHits(5);  // its hits number cut; default = 1; default absolute min. value = 0;  // FIXME
  TrackFinder->SetMinTrackerNHits(0); // tstb+tstec hits number cut; default = 2; default absiolute min. value = 2;  // FIXME
  ...
  TrackFinder->SetHitResolution(kSpdIts,  'm',  50*mkm,  50*mkm); // BARREL: MAPS   (u=z,v=x)
  TrackFinder->SetHitResolution(kSpdIts,  's',  50*mkm,  50*mkm); // BARREL: DSSD   (u=z,v=x)
