...
{  
  ...
  fModule = BuildModule(mapper->AddPrefix("SideSVol"), mapper->GetBaseMaterial(),
...
TGeoVolume* SpdRsTB2::CreateRegularLayer(Int_t layerN, Double_t rsSize, Double_t rsThickness, Double_t rsLength) 
{
  ...
  if (layerN % 2 == 0) 
  {
      ...
      for (UInt_t i(0); i < Nenvelopes; i++) 
      {
           Layer_SideS_Vol->AddNode(CreateMDT(rsLength-0.2), i+1, new TGeoCombiTrans("", Layer_Envelope_shiftX, hmin + 3 + 3.5/2, 0, rotNoRot)); 
           Layer_Envelope_shiftX = Layer_Envelope_shiftX - widthEnv;
      }
  }
  else 
